REM Usage : shot-nvenc.bat [fileName] [interpolate] [encoder] [preset] [vmaf]
...
for /f "tokens=*" %%a in ( fps.txt ) do set fr=%%a
...
scenedetect -i %1 detect-adaptive -m 1.5 split-video -a "-an -vcodec libvpx-vp9 -lossless 1 -preset fast" -o %current_date%_splitted_video
echo.
